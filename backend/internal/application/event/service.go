...
func (s *Service) TransitionExpiredEvents(ctx context.Context) error {
	completedTransitions, err := s.eventRepo.TransitionEventStatuses(ctx)
	if err != nil {
		return err
	}
	for _, transition := range completedTransitions {
		s.evaluateParticipationBadgesForEvent(ctx, transition.EventID)
		s.evaluateHostBadges(ctx, transition.HostID)
	}
	return nil
}
